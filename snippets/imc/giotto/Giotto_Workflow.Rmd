...
Furthermore, the cell metadata can be stored within the `Giotto` object.
The package does not specify in which format the metadata should be stored.
But the function only works when it's supplied as `data.table`.
However neither the `show` or `print` method display information on the structure of the metadata.
