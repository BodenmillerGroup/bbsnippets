function [autoencoder, trainList]=CellShapeAutoencoder(FileList, train_frac, Masks)
...
for iFile = 1:numel(FileList)
    ...
    ID=[1:numel(Field)]';
    [filepath,name,ext] = fileparts(string(FileList{iFile}));
    %   Pos=repmat(str2num(FileList{iFile}(end-6:end-4)), numel(Field),1 );
    Pos=repmat(name, numel(Field),1);
